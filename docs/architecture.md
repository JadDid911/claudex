...
`events.jsonl` is append-only. `state.json` is atomically replaced and tracks the next event sequence, provider-private session handles, observed capacity, active turns, active Supermode stages, and lease state. A torn JSONL tail is truncated to its last valid record during replay.
...
Codex uses fresh `exec --json` invocations with an explicit working directory. Read turns pass an explicit read-only sandbox; write turns use `--approve-for-me`, which selects the CLI's workspace-write sandbox. Write-capable synthesis is deliberately fresh because the installed `exec resume` surface cannot reassert those safety flags. Claude may resume its provider-private session while reasserting the permission profile and a `Read,Glob,Grep`-only helper tool envelope on every read invocation.
...
The scheduler selects the executor first. The configured `plan`, `execute`, and `review` affinities then select each stage independently; their saved provider-specific model and effort overrides are resolved at invocation time. Missing, unavailable, or cooling affinities fall back to capacity-aware selection. Automatic plan and review prefer the provider other than the executor. A same-provider review is preserved when explicitly configured and used automatically only when no independent provider is eligible.
...
Each transition persists `workflow`, `pipelineStage`, and active-provider state. A standalone clarification question changes the turn to `waiting-for-user`; failure or cancellation records the terminal stage. All three conditions skip every downstream stage. Capacity or availability failures may retry once on another eligible provider only before side effects are possible, so an uncertain writer is never replayed.

## Single-writer invariant

The lease records provider, turn, acquisition time, and generation. A second writer cannot acquire it. Release occurs only after the provider operation ends on success, failure, timeout, cancellation, or shutdown.
